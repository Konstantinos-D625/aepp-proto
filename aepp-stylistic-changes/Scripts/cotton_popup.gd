...
# Επίσης, πάντα 1 Αριθμητικό Κλειδί τιμής 8 — το "bootstrap" κλειδί του side
# quest του κάστρου (βλ. Scripts/castle_popup.gd): ικανοποιεί ακριβώς τη
# συνθήκη εισόδου του Armory ("k <= 8"), η μοναδική πύλη χωρίς δικά της
# κλειδιά ήδη μέσα στο κάστρο. Ίδιο σύστημα/νόμισμα με το
# Scripts/key_inventory.gd, ώστε να φαίνεται και στην Αποθήκη.
...
func _build_bubble() -> Control:
	...
	const BY := 145.0
	const BW := 565.0
	# Αρκετό ύψος ώστε το Label (11 γραμμές × ~37px στο font 26) να χωράει στο
	# BH-130 — στα 500 η τελευταία γραμμή («Τι λες, τα καταφέρνεις;») κοβόταν.
	const BH := 560.0
