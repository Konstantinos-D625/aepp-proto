...
#
# ΚΑΝΟΝΑΣ: λάθος σε ΟΠΟΙΑΔΗΠΟΤΕ ερώτηση/level -> ΔΕΝ βγάζει τον παίκτη από
# την οθόνη. Δείχνει σύντομο μήνυμα λάθους/επανεκκίνησης και μετά από λίγο
# ξαναρχίζει ΑΥΤΟΜΑΤΑ από το Level 1 (Σ/Λ 1/3) — βλ. _restart_attempt(). Ο
# παίκτης έχει ΑΠΕΡΙΟΡΙΣΤΕΣ προσπάθειες μέσα στην ίδια μέρα, οπότε αυτό το
...
## απόπειρα ως ημιτελή (_finish με ό,τι levels είχαν ολοκληρωθεί πριν) — δεν
## έχει καμία επίδραση στο streak ή στη δυνατότητα να ξαναπροσπαθήσει (ο
## παίκτης έχει ούτως ή άλλως απεριόριστες προσπάθειες τη μέρα).
