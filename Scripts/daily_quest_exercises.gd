...
#
# ΚΑΝΟΝΑΣ: πρώτο λάθος σε ΟΠΟΙΑΔΗΠΟΤΕ ερώτηση/level -> χάνεις το Daily Quest
# ΓΙΑ ΤΗ ΜΕΡΑ, καμία 2η ευκαιρία. Το ίδιο ισχύει αν βγεις («◄ Πίσω») πριν
# ολοκληρώσεις — μετράει σαν να απέτυχες ΤΩΡΑ (αλλιώς θα μπορούσες να
# βγαίνεις/ξανάμπαίνεις απεριόριστα μέχρι να πέσεις σε εύκολες ερωτήσεις).
#
...
# η τοποθέτηση, ο γύρος (και το quest) αποτυγχάνει.
#
# Rewards (βλ. REWARD_TIERS): αθροιστικά ανάλογα με το πόσα levels
# ολοκληρώθηκαν ΠΛΗΡΩΣ πριν την αποτυχία (ή όλα τα 3, σε πλήρη επιτυχία).
# Το GameData.record_daily_quest_result() καταγράφει το αποτέλεσμα +
# ανανεώνει το streak (ΜΟΝΟ σε πλήρη επιτυχία) — δεν κάνει τίποτα άλλο.
...
# Αθροιστικά rewards ανά "πόσα levels ολοκληρώθηκαν πλήρως" (0 = καμία
# ανταμοιβή — απέτυχε μέσα στο Level 1). Ίδιο σχήμα {"currency","amount",
# "icon"(προαιρετικό)} με τα results arrays του cotton/blacksmith/miner_popup.gd.
const REWARD_TIERS := {
	1: [
		{ "currency": "Βαμβάκι", "amount": 1 },
		{ "currency": "Χρυσό",   "amount": 1 },
		{ "currency": "Σίδερο",  "amount": 1 },
	],
	2: [
		{ "currency": "Βαμβάκι", "amount": 5 },
		{ "currency": "Χρυσό",   "amount": 5 },
		{ "currency": "Σίδερο",  "amount": 5 },
	],
	3: [
		{ "currency": "Βαμβάκι", "amount": 5 },
		{ "currency": "Χρυσό",   "amount": 5 },
		{ "currency": "Σίδερο",  "amount": 5 },
		{ "currency": "Σφαίρα Δύναμης",   "amount": 1, "icon": "res://Εικόνες/red-orb.png" },
		{ "currency": "Σφαίρα Ταχύτητας", "amount": 1, "icon": "res://Εικόνες/green-orb.png" },
		{ "currency": "Σφαίρα Εξυπνάδας", "amount": 1, "icon": "res://Εικόνες/blue-orb.png" },
	],
}
...
## «◄ Πίσω» ΚΑΤΑ ΤΗ ΔΙΑΡΚΕΙΑ μιας απόπειρας μετράει σαν αποτυχία ΤΩΡΑ (με
## credit για όσα levels ολοκληρώθηκαν πριν) — αλλιώς θα μπορούσε κανείς να
## βγαίνει/ξανάμπαίνει απεριόριστα μέχρι να πέσει σε εύκολες ερωτήσεις,
## παρακάμπτοντας το "καμία 2η ευκαιρία".
...
func _on_answer_result(correct: bool) -> void:
	if correct:
		...
		_feedback.text = "✘  Λάθος! Έχασες το Daily Quest για σήμερα."
		var t := get_tree().create_timer(1.3)
		t.timeout.connect(func(): _finish(_level - 1))
...
func _build_matching_ui() -> void:
	...
	_left_col.add_theme_constant_override("separation", 14)
	...
	_right_col.add_theme_constant_override("separation", 14)
...
func _on_matching_round_completed(_round_index: int, _total_rounds: int, correct_count: int, pair_total: int, _earned: int, flags: Array) -> void:
	...
	if correct_count == pair_total:
		...
		_matching_feedback.text = "%d/%d σωστά — Έχασες το Daily Quest για σήμερα." % [correct_count, pair_total]
		var t2 := get_tree().create_timer(1.8)
		t2.timeout.connect(func(): _finish(2))
...
# ═══════════════════════════════════════════════════════════════════════════
# ΤΕΛΟΣ ΑΠΟΠΕΙΡΑΣ — καταγραφή + rewards
# ═══════════════════════════════════════════════════════════════════════════
func _finish(levels_completed: int) -> void:
	...
	var results: Array = REWARD_TIERS.get(clamped, [])
	for r in results:
		Currency.add(str(r["currency"]), int(r["amount"]))

	var title := "Δεν τα κατάφερες σήμερα..."
	if clamped >= 3:
		title = "Άριστα! Ολοκλήρωσες όλο το Daily Quest!"
	elif clamped > 0:
		title = "Ολοκλήρωσες %d από τα 3 levels!" % clamped
	_show_completion(title, results)
...
func _show_completion(title_text: String, results: Array) -> void:
	...
	if results.is_empty():
		...
		none_lbl.text = "Καμία ανταμοιβή σήμερα — ξαναδοκίμασε αύριο!"
		none_lbl.position = Vector2(px + 40, py + 200)
		none_lbl.size = Vector2(PW - 80, 80)
		none_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		none_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		none_lbl.add_theme_font_size_override("font_size", 26)
		none_lbl.add_theme_color_override("font_color", C_BONE_D)
		overlay.add_child(none_lbl)
	else:
		var header := Label.new()
		header.text = "Κέρδισες:"
		header.position = Vector2(px + 40, py + 190)
		header.size = Vector2(PW - 80, 50)
		header.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		header.add_theme_font_size_override("font_size", 28)
		header.add_theme_color_override("font_color", C_GOLD_D)
		overlay.add_child(header)

		_build_loot_rows(overlay, results, Vector2(px + 40, py + 250), PW - 80)
...
## Ίδιο μοτίβο icon-κάτω-από-κείμενο με cotton_popup.gd/blacksmith_popup.gd/
## miner_popup.gd (_make_loot_entry) — οι Σφαίρες δείχνουν εικονίδιο, τα
## υλικά μένουν σε απλό κείμενο. Δύο ΞΕΧΩΡΙΣΤΕΣ σειρές, η μία κάτω από την
## άλλη: πρώτα τα υλικά (χωρίς εικονίδιο), μετά οι Σφαίρες (με εικονίδιο) —
## όχι όλα ανακατεμένα στην ίδια σειρά.
func _build_loot_rows(parent: Control, results: Array, pos: Vector2, width: float) -> void:
	...
	if not materials.is_empty():
		col.add_child(_make_loot_row(materials))
	if not orbs.is_empty():
		col.add_child(_make_loot_row(orbs))

func _make_loot_row(items: Array) -> Control:
	var row := HBoxContainer.new()
	row.alignment = BoxContainer.ALIGNMENT_CENTER
	row.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_theme_constant_override("separation", 20)
	for item in items:
		row.add_child(_make_loot_entry(item))
	return row

func _make_loot_entry(item: Dictionary) -> Control:
	...
	var lbl := Label.new()
	lbl.text = "+%d %s" % [item["amount"], item["currency"]]
	lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
