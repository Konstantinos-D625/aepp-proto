...
# ── Μονοπάτια εικόνων ─────────────────────────────────────────────────────
# Ο ΧΑΡΤΟΓΡΑΦΟΣ της περιοχής ΕΠ (σπιτάκι «ΧΑΡΤΟΓΡΑΦΕΙΟ» στο ep.area.bg.png).
# ΙΔΙΟ ΜΟΤΙΒΟ ΜΕ ΤΗ ΒΟΤΑΝΟΥ/ΑΛΧΗΜΙΣΤΗ (botanist_popup.gd, alchemy_popup.gd):
# δύο full-screen εικόνες της ΙΔΙΑΣ σκηνής, με και χωρίς τον χαρακτήρα, που
# κάθονται pixel-πάνω-σε-pixel· στην Κατάσταση 2 η από πάνω κάνει fade out
# και «αποκαλύπτεται» το άδειο δωμάτιο — ο χαρτογράφος μοιάζει να φεύγει.
#
# ΠΡΟΣΟΧΗ: εδώ η ονομασία είναι ΑΝΤΙΣΤΡΟΦΗ από τα bota.*/alch.* — το «bg» έχει
# τον χαρακτήρα και το «bg2» είναι το άδειο δωμάτιο. Μην τα ανταλλάξεις.
const BG_PATH    := "res://Εικόνες/charto.bg2.png"  # χωρίς τον χαρτογράφο
const CHAR_PATH  := "res://Εικόνες/charto.bg.png"   # με τον χαρτογράφο
const BOARD_PATH := "res://Εικόνες/board.png"

# ── Παλέτα (χαρτογραφείο — ναυτικό μπλε, μπρούντζινες πυξίδες) ────────────
# Τα αδελφά popup χρησιμοποιούν ΧΡΥΣΗ (φούρναρης), ΒΙΟΛΕΤΙ (αλχημιστής) ή
# ΠΡΑΣΙΝΗ (βοτανού) πινελιά· εδώ ο τόνος είναι ΓΑΛΑΖΙΟΣ ώστε να δένει με τα
# λάβαρα και τους χάρτες, αλλά τα σχήματα/μεγέθη/αποστάσεις μένουν ακριβώς
# τα ίδια για συνεπές UI.
const C0       := Color(0, 0, 0, 0)
const C_AZ     := Color(0.360, 0.640, 0.900)   # γαλάζια πινελιά
const C_AZ_D   := Color(0.105, 0.215, 0.395)
const C_AZ_S   := Color(0.760, 0.890, 1.000)
const C_PARCH  := Color(0.968, 0.945, 0.888)   # κιτρινωπό χαρτί παλιού χάρτη
const C_PARCH_D:= Color(0.800, 0.775, 0.720)
const C_GOLD   := Color(0.870, 0.700, 0.330)   # μπρούντζος πυξίδας
...
# ═══════════════════════════════════════════════════════════════════════════
# ΚΑΤΑΣΤΑΣΗ 2 — ο Χαρτογράφος φεύγει (fade στο άδειο δωμάτιο), board εμφανίζεται
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Χαρακτήρας Χαρτογράφος — full-screen layer «σκηνή με τον χαρτογράφο» ───
# ΙΔΙΟ expand/stretch με το _build_background ώστε οι δύο εικόνες (με/χωρίς
# τον χαρτογράφο) να κάθονται pixel-πάνω-σε-pixel — αλλιώς το fade της
...
func _build_bubble() -> Control:
	...
	# Ο χαρτογράφος στέκεται ΑΡΙΣΤΕΡΑ-ΚΕΝΤΡΟ πίσω από το τραπέζι, οπότε η
	# φούσκα πάει δεξιά, με ουρά κάτω-αριστερά προς αυτόν.
	# ΠΡΟΣΟΧΗ ΣΤΟ ΥΨΟΣ: τα μαλλιά του ξεκινούν στο y≈412 της οθόνης (η εικόνα
	# 1024×1536 μπαίνει με KEEP_ASPECT_COVERED σε 1080×1920, άρα κλίμακα 1.25
	# και οριζόντιο κόψιμο 100px ανά πλευρά) και τα μάτια του στο y≈519. Η
	# φούσκα ΠΡΕΠΕΙ να τελειώνει πάνω από το πρόσωπο.
	const BX := 430.0
	const BY := 26.0
	const BW := 620.0
	const BH := 356.0
	...
	var msg := Label.new()
	msg.text = "Χάρτες, παντού χάρτες!\n\nΚάθε νησί το μετράω βήμα-βήμα,\nξανά και ξανά, ώσπου να κλείσει!\n\nΛύσε τις ασκήσεις στον πίνακα!"
	...
	# Αστεράκια πυξίδας γύρω από τη φούσκα
	_compass_sparks(root, BX, BY, BW, BH)

	return root

# Τα σκαλοπάτια της ουράς κατεβαίνουν προς τα ΑΡΙΣΤΕΡΑ (εκεί στέκεται ο
# χαρτογράφος μέσα στη σκηνή).
func _bubble_tail(parent: Control, tx: float, ty: float) -> void:
	...
	_cr_on(parent, Vector2(tx-1,  ty-1),  Vector2(34,  5), C_AZ)
	_cr_on(parent, Vector2(tx-4,  ty+2),  Vector2(5,  14), C_AZ)

func _compass_sparks(parent: Control, bx: float, by: float, bw: float, bh: float) -> void:
	...
	for _i in range(10):
		...
		# Εναλλάσσει γαλάζιο και μπρούντζινο (λάβαρα και πυξίδες)
		s.bg_color = C_AZ_S if rng.randf() > 0.4 else C_GOLD
...
func _make_answer_button(text: String, accent: Color) -> Button:
	...
	dis.border_color = C_AZ_D.darkened(0.25)
...
func _style_back_btn(btn: Button) -> void:
	var n := StyleBoxFlat.new()
	n.bg_color = C_WOOD_D; n.border_color = C_AZ.darkened(0.15)
	n.set_border_width_all(4); n.set_corner_radius_all(10)
	n.shadow_color = Color(0,0,0,0.68); n.shadow_size = 7
	btn.add_theme_stylebox_override("normal", n)
	var h := StyleBoxFlat.new()
	h.bg_color = C_WOOD; h.border_color = C_AZ
	h.set_border_width_all(5); h.set_corner_radius_all(10)
	h.shadow_color = C_AZ.lightened(0.10); h.shadow_size = 16
	btn.add_theme_stylebox_override("hover", h)
	var pr := StyleBoxFlat.new()
	pr.bg_color = Color(0.022,0.040,0.070); pr.border_color = C_AZ.darkened(0.25)
	pr.set_border_width_all(3); pr.set_corner_radius_all(10)
	btn.add_theme_stylebox_override("pressed", pr)
	btn.add_theme_stylebox_override("focus", StyleBoxFlat.new())
	btn.add_theme_color_override("font_color",         C_AZ_S)
	btn.add_theme_color_override("font_hover_color",   Color(1,1,1))
	btn.add_theme_color_override("font_pressed_color", C_AZ.darkened(0.30))
