...
# ── Μονοπάτια εικόνων ─────────────────────────────────────────────────────
# Η ΒΟΤΑΝΟΥ της περιοχής ΕΠ (σπιτάκι «ΒΟΤΑΝΟΠΩΛΕΙΟ» στο ep.area.bg.png).
# ΙΔΙΟ ΜΟΤΙΒΟ ΜΕ ΤΟΝ ΑΛΧΗΜΙΣΤΗ/ΦΟΥΡΝΑΡΗ (alchemy_popup.gd, bakery_popup.gd):
# το bota.bg2.png είναι ΟΛΟΚΛΗΡΗ σκηνή (η βοτανού ΜΑΖΙ με το βοτανοπωλείο)
# και το bota.bg.png είναι η ΙΔΙΑ ακριβώς σκηνή ΧΩΡΙΣ αυτήν. Γι' αυτό το
# _build_character δεν τοποθετεί μικρό TextureRect, αλλά ΔΕΥΤΕΡΟ full-screen
# layer πάνω από το BG_PATH: όταν στην Κατάσταση 2 το _char κάνει fade out,
# «αποκαλύπτεται» το άδειο μαγαζί από κάτω — η βοτανού μοιάζει να φεύγει, το
# δωμάτιο μένει. Οι δύο εικόνες είναι και οι δύο 1024×1536, άρα κάθονται
# pixel-πάνω-σε-pixel.
const BG_PATH    := "res://Εικόνες/bota.bg.png"    # χωρίς τη βοτανού
const CHAR_PATH  := "res://Εικόνες/bota.bg2.png"   # με τη βοτανού
const BOARD_PATH := "res://Εικόνες/board.png"

# ── Παλέτα (βοτανοπωλείο — πράσινα φύλλα, γαλάζια μαγικά άνθη) ────────────
# Τα αδελφά popup χρησιμοποιούν ΧΡΥΣΗ (φούρναρης) ή ΒΙΟΛΕΤΙ (αλχημιστής)
# πινελιά· εδώ ο τόνος είναι ΠΡΑΣΙΝΟΣ ώστε να δένει με τα βότανα, αλλά τα
# σχήματα/μεγέθη/αποστάσεις μένουν ακριβώς τα ίδια για συνεπές UI.
const C0       := Color(0, 0, 0, 0)
const C_GRN    := Color(0.400, 0.720, 0.400)   # πράσινη πινελιά
const C_GRN_D  := Color(0.135, 0.295, 0.175)
const C_GRN_S  := Color(0.780, 0.960, 0.790)
const C_PARCH  := Color(0.972, 0.962, 0.925)   # ζεστό λινό περγαμηνής
const C_PARCH_D:= Color(0.800, 0.790, 0.745)
const C_SKY    := Color(0.400, 0.700, 0.910)   # γαλάζιο μαγικού άνθους
const C_WOOD   := Color(0.170, 0.145, 0.110)
const C_WOOD_D := Color(0.100, 0.085, 0.062)
const C_TEXT   := Color(0.100, 0.130, 0.078)
...
# ═══════════════════════════════════════════════════════════════════════════
# ΚΑΤΑΣΤΑΣΗ 2 — η Βοτανού φεύγει (fade στο άδειο μαγαζί), board εμφανίζεται
# ═══════════════════════════════════════════════════════════════════════════
...
# ── Χαρακτήρας Βοτανού — full-screen layer «σκηνή με τη βοτανού» ───────────
# ΙΔΙΟ expand/stretch με το _build_background ώστε οι δύο εικόνες (με/χωρίς
# τη βοτανού) να κάθονται pixel-πάνω-σε-pixel — αλλιώς το fade της
...
func _build_bubble() -> Control:
	...
	# Η βοτανού στέκεται ΑΡΙΣΤΕΡΑ-ΚΕΝΤΡΟ μέσα στη σκηνή, οπότε η φούσκα πάει
	# δεξιά, με ουρά κάτω-αριστερά προς αυτήν.
	# ΠΡΟΣΟΧΗ ΣΤΟ ΥΨΟΣ: το ανθοστέφανό της ξεκινά στο y≈456 της οθόνης (η
	# εικόνα 1024×1536 μπαίνει με KEEP_ASPECT_COVERED σε 1080×1920, άρα
	# κλίμακα 1.25 και οριζόντιο κόψιμο 100px ανά πλευρά). Η φούσκα ΠΡΕΠΕΙ
	# να τελειώνει πάνω από εκεί, αλλιώς σκεπάζει το πρόσωπό της.
	const BX := 430.0
	const BY := 30.0
	const BW := 620.0
	const BH := 380.0
	...
	var msg := Label.new()
	msg.text = "Καλώς ήρθες στο Βοτανοπωλείο!\n\nΚάθε πρωί ποτίζω τα βότανα —\nξανά και ξανά, ώσπου ν' ανθίσουν!\n\nΛύσε τις ασκήσεις στον πίνακα!"
	...
	# Γύρη/σπόροι που αιωρούνται γύρω από τη φούσκα
	_pollen_motes(root, BX, BY, BW, BH)

	return root

# Τα σκαλοπάτια της ουράς κατεβαίνουν προς τα ΑΡΙΣΤΕΡΑ (εκεί στέκεται η
# βοτανού μέσα στη σκηνή).
func _bubble_tail(parent: Control, tx: float, ty: float) -> void:
	...
	_cr_on(parent, Vector2(tx-1,  ty-1),  Vector2(34,  5), C_GRN)
	_cr_on(parent, Vector2(tx-4,  ty+2),  Vector2(5,  14), C_GRN)

func _pollen_motes(parent: Control, bx: float, by: float, bw: float, bh: float) -> void:
	...
	for _i in range(10):
		...
		# Εναλλάσσει πράσινο και γαλάζιο (φύλλα και μαγικά άνθη)
		s.bg_color = C_GRN_S if rng.randf() > 0.4 else C_SKY
...
func _make_answer_button(text: String, accent: Color) -> Button:
	...
	dis.border_color = C_GRN_D.darkened(0.25)
...
func _style_back_btn(btn: Button) -> void:
	var n := StyleBoxFlat.new()
	n.bg_color = C_WOOD_D; n.border_color = C_GRN.darkened(0.15)
	n.set_border_width_all(4); n.set_corner_radius_all(10)
	n.shadow_color = Color(0,0,0,0.68); n.shadow_size = 7
	btn.add_theme_stylebox_override("normal", n)
	var h := StyleBoxFlat.new()
	h.bg_color = C_WOOD; h.border_color = C_GRN
	h.set_border_width_all(5); h.set_corner_radius_all(10)
	h.shadow_color = C_GRN.lightened(0.10); h.shadow_size = 16
	btn.add_theme_stylebox_override("hover", h)
	var pr := StyleBoxFlat.new()
	pr.bg_color = Color(0.030,0.055,0.032); pr.border_color = C_GRN.darkened(0.25)
	pr.set_border_width_all(3); pr.set_corner_radius_all(10)
	btn.add_theme_stylebox_override("pressed", pr)
	btn.add_theme_stylebox_override("focus", StyleBoxFlat.new())
	btn.add_theme_color_override("font_color",         C_GRN_S)
	btn.add_theme_color_override("font_hover_color",   Color(1,1,1))
	btn.add_theme_color_override("font_pressed_color", C_GRN.darkened(0.30))
