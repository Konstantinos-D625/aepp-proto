...
# είναι ο ΜΟΝΑΔΙΚΟΣ NPC που δίνει Σφαίρα Ταχύτητας (Cottonman -> Εξυπνάδας,
# Blacksmith -> Δύναμης, βλ. αντίστοιχα σχόλια εκεί). Ίδιες προϋποθέσεις με
# το κανονικό loot παραπάνω (δίνεται μαζί με το χρυσό στο _finish).
const SPHERE_NAME := "Σφαίρα Ταχύτητας"
const SPHERE_ICON := "res://Εικόνες/green-orb.png"
...
func _build_character() -> TextureRect:
	...
	# Οριζόντια κεντραρισμένο στην οθόνη (1080×1920)· κάθετα κάτω από το
	# κέντρο (+230px) ώστε να μην τον καλύπτει η φούσκα ομιλίας.
	char_rect.position     = Vector2((1080.0 - 510.0) / 2.0, (1920.0 - 980.0) / 2.0 + 230.0)
...
func _finish(correct_count: int, total: int, gold: int) -> void:
	...
	var results := [
		{ "name": "Χρυσό", "amount": gold },
		{ "name": SPHERE_NAME, "amount": 1, "icon": SPHERE_ICON },
	]
...
## bullet-κείμενο (π.χ. Χρυσό, που δεν έχει ακόμα δικό του εικονίδιο εδώ).
