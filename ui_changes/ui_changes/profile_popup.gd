...
#
# ΜΕΓΕΘΗ: το popup είναι στο αρχικό (μικρό) μέγεθος, μεγεθυμένο κατά +10% σε ΟΛΑ
# (κάρτα, γραμματοσειρές, κουμπιά, περιθώρια) — αίτημα χρήστη για λίγο πιο
# ευανάγνωστη/ευκολοπάτητη οθόνη χωρίς να αλλάξει η αναλογία/διάταξη.
...
func _build() -> void:
	...
	card.offset_left = -484.0; card.offset_top = -660.0
	card.offset_right = 484.0; card.offset_bottom = 660.0
	...
	margin.add_theme_constant_override("margin_top", 51)
	...
	emblem.custom_minimum_size = Vector2(176, 176)
	...
	_name_label.add_theme_font_size_override("font_size", 51)
	...
	# ── Κουμπί λογαριασμού (Σύνδεση / Αποσύνδεση) — λίγο μεγαλύτερο (πατιέται) ──
	_account_btn = Button.new()
	_account_btn.custom_minimum_size = Vector2(0, 81)
	_account_btn.add_theme_font_size_override("font_size", 33)
	...
	_list.add_theme_constant_override("separation", 13)
	...
	close_btn.offset_left = -86.0; close_btn.offset_top = 18.0
	close_btn.offset_right = -20.0; close_btn.offset_bottom = 84.0
...
func _build_delete_overlay() -> void:
	...
	card.anchor_left = 0.5; card.anchor_top = 0.5
	card.anchor_right = 0.5; card.anchor_bottom = 0.5
	card.offset_left = -440.0; card.offset_top = -330.0
	card.offset_right = 440.0; card.offset_bottom = 330.0
	var csb := StyleBoxFlat.new()
	...
	_delete_input.custom_minimum_size = Vector2(0, 70)
	_delete_input.add_theme_font_size_override("font_size", 31)
	...
	_delete_status.add_theme_font_size_override("font_size", 24)
	...
	cancel.text = "Άκυρο"
	cancel.custom_minimum_size = Vector2(176, 70)
	...
	_delete_confirm_btn.custom_minimum_size = Vector2(286, 70)
	_delete_confirm_btn.add_theme_font_size_override("font_size", 29)
...
## Tab κουμπί «Στοιχεία/Επιτεύγματα» — λίγο μεγαλύτερο (font+ύψος) από το σώμα, ώστε
## να πατιέται πιο εύκολα (ρητό αίτημα χρήστη).
...
func _populate_stats() -> void:
	...
	if Net.is_logged_in():
		...
		# Κουμπί διαγραφής — λίγο μεγαλύτερο (font+ύψος) ώστε να πατιέται πιο εύκολα.
		var del_btn := Button.new()
		del_btn.text = "🗑  Διαγραφή λογαριασμού"
		del_btn.custom_minimum_size = Vector2(0, 88)
		del_btn.add_theme_font_size_override("font_size", 35)
...
func _stat_row(icon_key: String, label: String, value: String, value_color: Color = C_GOLD) -> Control:
	...
	if icon_tex:
		...
		ic.add_theme_font_size_override("font_size", 33)
		ic.custom_minimum_size = Vector2(51, 0)
...
func _ach_row(a: Dictionary) -> Control:
	...
	name_l.add_theme_font_size_override("font_size", 31)
